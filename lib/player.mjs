 ...
 * direction the input wants this tick (or null); it replaces the buffered
 * wish, which persists until it can be taken. Asking for the direction
 * already being travelled is a no-op that leaves the buffer alone, so a held
 * key does not cancel a tapped pre-turn. Returns { player, moved } with a new
 * player object; `moved` is the distance travelled in px (snaps excluded).
 */
export function stepPlayer(player, board, wantDir, speedTilesPerS, dt) {
  let x = player.x, y = player.y, dir = player.dir;
  const asked = wantDir !== null && wantDir !== undefined && DIRS[wantDir] !== undefined && wantDir !== dir;
  let want = asked ? wantDir : player.wantDir;
