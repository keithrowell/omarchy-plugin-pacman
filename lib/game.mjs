...
import { createPlayer, stepPlayer, bufferWant, tileOf } from "./player.mjs";
...
export function step(state, input, dt) {
  ...
  if (state.pauseTicks > 0) {
    ...
    // wipe a tapped pre-turn on a pellet tick.
    next.pauseTicks = state.pauseTicks - 1;
    const want = bufferWant(state.player, wantDir);
    if (want !== state.player.wantDir) next.player = Object.assign({}, state.player, { wantDir: want });
