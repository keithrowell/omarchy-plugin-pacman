...
import { fruitForLevel, fruitTile, FRUIT_SPAWN_COUNTS, FRUIT_TICKS, FRUIT_SCORE_TICKS } from "./fruit.mjs";
...
/** Pellets eaten so far this level (both kinds); survives death, only reset by advanceLevel. */
export function pelletsEaten(state) {
  return state.maze.pellets.length + state.maze.powerPellets.length - state.pelletsLeft;
}
...
function eat(next, events) {
  ...
  // The level's fruit appears at the 70th and the 170th pellet of the level,
  // each exactly once (pelletsEaten passes each count once per level; death
  // does not reset the board, so it cannot double-spawn on a restart).
  const eaten = pelletsEaten(next);
  if (FRUIT_SPAWN_COUNTS.indexOf(eaten) !== -1) {
    const f = fruitForLevel(next.level);
    next.fruit = { kind: f.kind, ticksLeft: FRUIT_TICKS };
    events.push({ type: "fruit", kind: f.kind });
  }
...
/** The fruit on the board, if Pac-Man's tile matches it: score, a point popup, then it is gone. */
function eatFruit(next, events) {
  if (next.fruit === null) return;
  const tile = tileOf(next.player, next.board);
  const spot = fruitTile(next.maze);
  if (!sameTile(tile, spot)) return;
  const kind = next.fruit.kind;
  const points = fruitForLevel(next.level).score;
  next.score = next.score + points;
  next.fruitScore = { kind, score: points, ticksLeft: FRUIT_SCORE_TICKS };
  next.fruit = null;
  events.push({ type: "fruit-eaten", kind, score: points });
}

/**
 * The fruit and its point popup count down only on a tick where they already
 * existed at its start (the spawn and eat ticks do not count down); each
 * clears itself at zero. No event; the arcade has none either.
 */
function tickFruit(next, state) {
  if (next.fruit !== null && next.fruit === state.fruit) {
    const ticksLeft = next.fruit.ticksLeft - 1;
    next.fruit = ticksLeft > 0 ? Object.assign({}, next.fruit, { ticksLeft }) : null;
  }
  if (next.fruitScore !== null && next.fruitScore === state.fruitScore) {
    const ticksLeft = next.fruitScore.ticksLeft - 1;
    next.fruitScore = ticksLeft > 0 ? Object.assign({}, next.fruitScore, { ticksLeft }) : null;
  }
}
 ...
 * extra-life, mode, fruit, fruit-eaten. `highScore` follows the score live,
 * so the HUD shows a beaten record as it happens.
 */
