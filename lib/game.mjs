 ...
 * the board empty of ghosts, for tests of the player alone).
 */
...
export function resetGame(state) {
  const fresh = createState(state.maze, { seed: state.rng });
  fresh.highScore = Math.max(state.highScore, state.score);
  return fresh;
}
